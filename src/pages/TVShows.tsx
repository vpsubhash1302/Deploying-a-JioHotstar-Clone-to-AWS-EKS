...
const TVShows = () => {
  ...
  const filteredShows = allShows.filter(show => {
    if (filters.genre && show.genre !== filters.genre) return false;
    if (filters.year && show.year !== filters.year) return false;
    if (filters.rating && parseFloat(show.rating || '0') < parseFloat(filters.rating)) return false;
    return true;
  });
  ...
  return (
    <div className="min-h-screen bg-black">
      {/* Hero Section */}
      <div className="relative h-64 bg-gradient-to-r from-purple-900 to-pink-900">
        <div className="absolute inset-0 bg-black/50"></div>
        <div className="relative z-10 flex items-center justify-center h-full">
          <div className="text-center">
            <h1 className="text-4xl md:text-6xl font-bold text-white mb-4">TV Shows</h1>
            <p className="text-xl text-gray-200">Binge-watch the best series and shows</p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 space-y-4 sm:space-y-0">
          <div className="flex items-center space-x-4">
            ...
            <span className="text-gray-400">{filteredShows.length} shows</span>
          </div>
        ...
        <div className="flex gap-8">
          ...
          <div className="flex-1">
            <ContentGrid items={filteredShows} viewMode={viewMode} />
          </div>
        </div>
      </div>
    </div>
